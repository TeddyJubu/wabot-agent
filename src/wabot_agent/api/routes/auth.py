"""Login routes — HTML GET and form POST.

Carved out of api/__init__.py as part of MASTER ME-1 Part 3. These routes
handle the operator-token + dashboard-password login flow that protects
the dashboard slide-overs. The CF Access path (when configured) is a
separate auth boundary at the reverse-proxy layer; these routes are the
fallback when no CF Access cookie is present.
"""

from __future__ import annotations

from fastapi import APIRouter, Depends, Form, HTTPException, Query, Request
from fastapi.responses import HTMLResponse, RedirectResponse, Response

from ...auth import (
    AuthIdentity,
    mint_operator_session_cookie,
    password_grants_dashboard_access,
    render_login_page,
    resolve_human_factory,
)
from ..dependencies import _safe_next_path
from ..deps import AppDeps


def register_auth_routes(router: APIRouter, deps: AppDeps) -> None:
    settings = deps.settings
    resolve_human = resolve_human_factory(settings)

    @router.get("/login", include_in_schema=False, response_model=None)
    async def login_page(
        request: Request,
        identity: AuthIdentity | None = Depends(resolve_human),  # noqa: B008
        next_path: str = Query("/", alias="next"),
        err: str | None = Query(None),
    ) -> Response:
        if identity is not None:
            return RedirectResponse(url=_safe_next_path(next_path), status_code=302)
        error_html = f'<p class="err">{err}</p>' if err else ""
        return HTMLResponse(render_login_page(error_html=error_html, next_path=next_path))

    @router.post("/api/auth/login", include_in_schema=False, response_model=None)
    async def login_submit(
        request: Request,
        password: str = Form(...),
        next_path: str = Form("/", alias="next"),
    ) -> Response:
        if not settings.operator_token:
            raise HTTPException(status_code=503, detail="operator token not configured")
        safe_next = _safe_next_path(next_path)
        if not password_grants_dashboard_access(settings, password):
            return HTMLResponse(
                render_login_page(
                    error_html='<p class="err">Wrong password.</p>',
                    next_path=safe_next,
                ),
                status_code=401,
            )
        response = RedirectResponse(url=safe_next, status_code=303)
        mint_operator_session_cookie(response, request, settings)
        return response
