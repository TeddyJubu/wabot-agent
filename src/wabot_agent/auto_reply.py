...
async def deliver_inbound_empty_reply(
    *,
    settings: Settings,
    wabot: WabotClient,
    inbound: InboundMessage,
) -> dict[str, Any]:
    """Send a short fallback when the model returns no text (Codex flake / empty turn)."""
    if not settings.auto_reply_enabled:
        return {"sent": False, "reason": "disabled"}
    if inbound.is_group and not settings.group_auto_reply_enabled:
        return {"sent": False, "reason": "group_auto_reply_disabled"}
    ...
    text = (
        "Sorry — I didn't get a reply out that time. "
