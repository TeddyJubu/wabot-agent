...
import { updateMcpServer, type McpServerRow } from "@/api/mcp";

interface Props {
  server: McpServerRow;
  onSaved: (server: McpServerRow) => void;
  onCancel: () => void;
}

export function EditMcpServerForm({ server, onSaved, onCancel }: Props) {
  const [name, setName] = useState(server.name);
  const [transport, setTransport] = useState(server.transport);
  const [configJson, setConfigJson] = useState(server.config_json);
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-3 rounded-card border border-border bg-bg-app px-3 py-3">
      ...
      <div className="space-y-1">
        <label className="text-[10px] text-fg-muted" htmlFor={`edit-mcp-name-${server.id}`}>Name</label>
        <input
      ...
      <div className="space-y-1">
        <label className="text-[10px] text-fg-muted" htmlFor={`edit-mcp-transport-${server.id}`}>Transport</label>
        <select
          id={`edit-mcp-transport-${server.id}`}
          value={transport}
      ...
      <div className="space-y-1">
        <label className="text-[10px] text-fg-muted" htmlFor={`edit-mcp-config-${server.id}`}>
          Config JSON
        </label>
        <textarea
          id={`edit-mcp-config-${server.id}`}
          value={configJson}
